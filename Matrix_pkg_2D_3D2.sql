...
subtype point_ty      is  number(12, 4);
type line_ty          is table of  point_ty index by pls_integer;
...
function  to_matrix_3D (p_x11 in number, p_x12 in number, p_x13 in number, p_x21 in number, p_x22 in number, p_x23 in number, p_x31 in number, p_x32 in number, p_x33 in number) return matrix_ty;
...
exception when others then
  util.show_error('Error in function to_matrix_2D.' , sqlerrm);
end to_matrix_2D;
...
function multiply_2D (p_matrix in matrix_ty, p_point in point_ty_2D) return point_ty_2D
is
l_point point_ty_2D := to_point_2D (0, 0);
begin
...
function multiply_3D (p_matrix in matrix_ty, p_point in point_ty_3D) return point_ty_3D
is
l_point point_ty_3D := to_point_3D (0, 0, 0);
begin
...
elsif p_matrix(1).count = 2 and p_matrix.count = 2 then return p_matrix(1)(1) * p_matrix(2)(2) - p_matrix(2)(1) * p_matrix(1)(2);
else
...
--
-- Show coordinates of a 3Dpoint on the screen
--
procedure print_point_3D (p_point point_ty_3D)
is
begin
